...
- **Check:** `node scripts/walker-blindvlekken.mjs | grep marge` — **40** = de klasse leeft (de 42 schermstories; over alle 257 stories zijn het er 57, met `--alle`); en `node -e` op `build-spec.json`: som van de kinderen tegen de ouderhoogte op `WorkoutDetailScreen/Playground` → 904 tegen 932.
...
- **Check:** `pnpm --filter rowtrack figma:check` moet 14/14 blijven en `node scripts/walker-blindvlekken.mjs` moet dalen op de assen die je aanpakte — rand 110, placeholder 4, gescrold 11, overloop 15, inline 3, marge 40 zijn de basislijn van 2026-09-09.
- **Status:** open
