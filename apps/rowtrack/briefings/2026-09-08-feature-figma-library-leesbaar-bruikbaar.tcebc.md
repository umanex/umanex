...
- [x] **R01 (P1)** CLAUDE.md:260 — Verify-pad beweert dat de node:test-suites niet in CI draaien — ze draaien er sinds 2026-08-10
      *Opgelost* — bewijs: gecorrigeerd; nagemeten `node --experimental-strip-types --test $(git ls-files "*.test.ts")` → 57 tests, 6 suites, exit 0, en de stap staat op `ci.yml:79`
...
- [x] **R02 (P1)** scripts/figma-poort-selftest.mjs:80 — bouwhash sorteert zijn delen en codeert geen boomstructuur — herordenen en herparenteren zijn onzichtbaar handwerk
      *Opgelost* — bewijs: elk deel draagt nu zijn boompad; drie structuurmutaties erbij in `figma:poort:selftest` (19/19), en met het sorteren terug vallen er twee van om
...
- [x] **R03 (P1)** scripts/figma-sync-check.mjs:416 — Een overgeslagen as (sla()) eindigt op exit 0 en de slotregel somt alsnog alle dertien assen op
      *Opgelost* — bewijs: de slotregel komt uit `checks` zelf en een overgeslagen as geeft **exit 2**; twee mutaties erbij (`onvolledig`, `onvolledig-laagnamen`) die exit 2 eisen
...
- [x] **R04 (P1)** scripts/figma-sync-check.mjs:280 — [link] is groen per constructie zodra figma:links gedraaid is — de as raakt Figma nooit
      *Opgelost* — bewijs: de as zegt nu wat hij meet — "gelijk aan de primary-id in het manifest van <datum> (dat de node in Figma leeft, meet deze as niet)"
...
- [x] **R05 (P1)** scripts/figma-sync-check.mjs:335 — [publicatie] meet de mtime van een git-getrackt bestand op dag-resolutie — mist het echte venster, vuurt op een verse checkout
      *Opgelost* — bewijs: commit-tijden in plaats van mtimes, met een aparte melding bij onvastgelegd werk; de tegenproef legt de twee bestanden nu in git vast in beide volgordes
...
- [x] **R06 (P1)** scripts/laagnamen.mjs:122 — [laagnaam] instabiel-subas is per constructie leeg: de producent normaliseert precies wat de guard toetst
      *Opgelost* — bewijs: `instabielePosities` erbij als ratel — de INVOER van `stabiliseer()`, die de producent niet wegnormaliseert. Staat op 2 (DeviceRow:1, KPI:1); twee mutaties erbij
...
- [x] **R07 (P1)** scripts/laagnamen.mjs:105 — vouwAlternatieven vouwt élk paar sleutels dat nooit samen op één node staat, niet alleen ternary-alternatieven — verschillende lagen krijgen dezelfde naam
      *Opgelost* — bewijs: vouwen vraagt nu POSITIEF bewijs (twee sleutels winnen om de beurt op dezelfde positie in verschillende varianten). Ouders met ≥2 identieke kindnamen 295 → 145, `gefold` 633 → 24, en `ErrorState → title` / `Button → text` blijven
...
- [x] **R08 (P1)** scripts/laagnamen.mjs:202 — De 'genest component'-tak noemt lagen naar componenten die niet in de boom staan
      *Opgelost* — bewijs: de tak eist twee gedeelde klassen én twee winnende nodes uit dezelfde bron in de subboom. 27 → 12 paren, alle twaalf tegen de broncode getoetst
...
- [x] **R09 (P2)** briefings/2026-09-08-feature-figma-library-leesbaar-bruikbaar.tcebc.md:89 — Afgevinkt acceptatie-item telt 30 slots; de artefacten geven er 24
      *Opgelost* — bewijs: nagemeten: **24** slots over 16 componenten, identiek in `figma/build-spec.min.json` en in `componentProperties` van het manifest; de briefing zei 30
...
- [x] **R10 (P2)** briefings/2026-09-08-feature-figma-library-leesbaar-bruikbaar.tcebc.md:25 — ELEMENTS zegt 33 component sets; het zijn er 15, en de briefing spreekt zichzelf tegen
      *Opgelost* — bewijs: nagemeten: **15** sets en **18** losse componenten
...
- [x] **R11 (P2)** briefings/2026-09-08-feature-figma-library-leesbaar-bruikbaar.tcebc.md:63 — De herijkingsroute van de drempel wijst naar een veld dat niet in figma/laagnamen.json staat
      *Opgelost* — bewijs: de route wijst nu naar `--hernoem` plus `perBron`/`echteNaamPct`, velden die er wél staan
...
- [x] **R12 (P2)** CLAUDE.md:254 — Verify-pad noemt breedte als parity-as, terwijl parity breedte expliciet uitsluit
      *Opgelost* — bewijs: breedte staat er nu expliciet NIET in, mét de reden en het gemeten voorbeeld
...
- [x] **R13 (P2)** scripts/figma-sync-check.mjs:293 — [hardcoded] herkent één van vier kleurnotaties; de as claimt 'zonder hardcoded hex'
      *Opgelost* — bewijs: vier notaties (3/4/6/8-cijferige hex + rgb/rgba/hsl); tegenproef met `#f0a`, `#AABBCCDD` en `rgb(255,0,0)` geeft twee FAILs waar het er nul waren
...
- [x] **R14 (P2)** scripts/figma-sync-check.mjs:307 — [binding] telt unieke waardestrings over de hele app, niet gaten — een nieuw gat met een bekende waarde is onzichtbaar
      *Opgelost* — bewijs: `BEKENDE_VOORKOMENS = 1906` als tweede ratel naast de unieke telling
...
- [x] **R15 (P2)** scripts/figma-sync-check.mjs:214 — [tokenwaarde]: geteld telt bezochte entries, niet vergeleken entries — de twee fontFamily-variabelen worden nooit getoetst
      *Opgelost* — bewijs: `geteld++` staat nu per vergelijkingstak, fontFamilies worden tegen `expoBase` getoetst, en de as faalt als niet elke bron-variabele vergeleken is
...
- [x] **R16 (P2)** scripts/figma-sync-check.mjs:389 — [instancevulling]: één pagina met een vullingsveld schakelt de as van 'overgeslagen' naar 'groen' voor alle 33
      *Opgelost* — bewijs: `some` in plaats van `every`: één pagina zonder vullingsveld slaat de hele as over
...
- [x] **R17 (P2)** figma/builder.js:337 — bouwhash sorteert zijn onderdelen en is daardoor blind voor herstructurering — poort() laat handwerk wissen
      *Opgelost* — bewijs: zelfde fix als hierboven — `figma:poort:selftest` 19/19 met `omgedraaid`, `verplaatst` en `omgewisseld`
...
- [x] **R18 (P2)** figma/library-migratie.js:174 — Een falende importVariableByKeyAsync in zetVerf/zetEffecten gooit alle voortgang van de run weg en zet de migratie permanent vast
      *Opgelost* — bewijs: `zetVerf` en `zetEffecten` zitten nu allebei in een try/catch, zodat een rejection in `meldingen` landt in plaats van de voortgangsvlag over te slaan
...
- [x] **R19 (P2)** scripts/laagnamen.mjs:159 — terugval() leest n.styleRef, maar de walker schrijft node.tekst.styleRef — de text-style-tak is dood
      *Opgelost* — bewijs: leest nu `n.tekst.styleRef`
...
- [x] **R20 (P3)** briefings/2026-09-08-feature-figma-library-leesbaar-bruikbaar.tcebc.md:111 — Bewijs citeert een telling die figma:check niet uitvoert
      *Opgelost* — bewijs: het bewijs wijst nu naar `figma/laagnamen.json` met het letterlijke commando
...
- [x] **R21 (P3)** briefings/2026-09-08-feature-figma-library-leesbaar-bruikbaar.tcebc.md:75 — De breuk achter de 75,1% laagnaamdekking reconstrueert niet uit laagnamen.json
      *Opgelost* — bewijs: gecorrigeerd naar 1 454/1 935, en `perBron` sommeert tot exact `nodes`
...
- [x] **R22 (P3)** briefings/2026-09-08-feature-figma-library-leesbaar-bruikbaar.tcebc.md:97 — Acceptatie-item telt drie brekende instancevulling-mutaties; het zijn er twee
      *Opgelost* — bewijs: gecorrigeerd naar twee
...
- [x] **R23 (P3)** scripts/figma-sync-check.mjs:203 — Guard-meldingen verwijzen nog naar het schema-2-recept dat CLAUDE.md nu schema 3 noemt
      *Opgelost* — bewijs: de melding noemt nu schema 3
...
- [x] **R24 (P3)** .storybook/main.ts:95 — De __RNW_SRC__-transform is lexicaal blind: hij herschrijft `StyleSheet.create(` ook in strings, comments en JSX-tekst
      *Opgelost* — bewijs: de vervanging kijkt naar de voorafgaande tekens en de build waarschuwt bij elk voorkomen in een string of comment
...
- [x] **R25 (P3)** scripts/laagnamen.mjs:26 — componentVan() strips maar één extensie, dus story-bestanden worden laagnamen als 'BottomSheet.stories'
      *Opgelost* — bewijs: `.stories.tsx` levert `null`; `BottomSheet.stories` en `Skeleton.stories` zijn weg uit de spec
