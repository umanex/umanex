...
|---|---|
| **Componenten vastleggen (build-pad)** | `pnpm --filter rowtrack build-storybook` + `pnpm --filter rowtrack render:sweep` — rendert álle 197 stories in Chromium en telt console-fouten én lege renders. Dit is het enige render-pad dat zonder simulator werkt. Een geslaagde build zegt hier niets: gemeten 2026-09-07 gaf `storybook build` exit 0 terwijl 26 stories leeg renderden. **Dit pad meet dev niet.** Gemeten 2026-09-08: 197/197 groen terwijl `storybook dev` in dezelfde commit niet eens startte. Gebruik hiernaast de dev-sweep. |
