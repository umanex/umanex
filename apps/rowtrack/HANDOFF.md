...
- **Bevinding:** De tekst-fix (hug/blok + uitlijning) is in de library én in de 24 schermframes gebouwd, maar een instance toont de **gepubliceerde** library-versie. De twaalf frames waarvan alle tekst in instances zit (ActivePhase ×6, IdlePhase ×4, ResetPassword ×2 — die laatste twee zaten al op de vloer) staan in de beeld-as daardoor op exact hetzelfde getal als vóór de fix. Daarbovenop: een vérse style-import uit een library met ongepubliceerde wijzigingen hangt (eigenaardigheid 9 in CLAUDE.md), dus `type/activeProgress`, `shadow/buttonOutline` en `type/labelGoalPrefix` zijn in de nagebouwde subbomen van de schermen nu **ongebonden** (losse fontwaarden, melding `niet te importeren`).
- **Check:** in RowTrack - Design via `figma_execute`: `(await figma.importStyleByKeyAsync('332d6d5667ebbc4ff595b66fc47aabf382b0ebea')).name` binnen 4 s = de library is gepubliceerd en de import werkt weer; hangt hij, dan is er nog niet gepubliceerd. En `node scripts/beeld-parity.mjs | grep ActivePhase__Playground` — staat er nog 3,04 %, dan zijn de schermen na de publicatie niet herbouwd.
